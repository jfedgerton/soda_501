...
#   (A) Oracle model: y ~ d + x_true
#   (B) Naive model:  y ~ d + x_obs
#   (C) Calibration:  estimate x_true ~ x_obs in a validation sample, predict x_hat
#   Also: outcome placebo: y_placebo ~ d + x_obs
...
validation_idx <- sample(seq_len(n), size = floor(validation_share * n), replace = FALSE)
is_validation <- seq_len(n) %in% validation_idx

# Storage
rows <- vector("list", length(sigma_u_grid))

cat("\n--- Running measurement error simulations ---\n")

for (s in seq_along(sigma_u_grid)) {
  sigma_u <- sigma_u_grid[s]

  tau_oracle_list  <- numeric(R)
  tau_naive_list   <- numeric(R)
  tau_cal_list     <- numeric(R)
  tau_placebo_list <- numeric(R)

  beta_oracle_list <- numeric(R)
  beta_naive_list  <- numeric(R)
  beta_cal_list    <- numeric(R)
  ...
  for (r in seq_len(R)) {
    ...
    tau_oracle_list[r]  <- coef(fit_oracle)["d"]
    beta_oracle_list[r] <- coef(fit_oracle)["x_true"]

    # (B) Naive regression: y ~ d + x_obs
    fit_naive <- lm(y ~ d + x_obs, data = df)
    tau_naive_list[r]  <- coef(fit_naive)["d"]
    beta_naive_list[r] <- coef(fit_naive)["x_obs"]

    # (C) Regression calibration
    ...
    tau_cal_list[r]  <- coef(fit_calibrated)["d"]
    beta_cal_list[r] <- coef(fit_calibrated)["x_hat"]

    # Outcome placebo: y_placebo ~ d + x_obs
    fit_placebo <- lm(y_placebo ~ d + x_obs, data = df)
    tau_placebo_list[r] <- coef(fit_placebo)["d"]
  }

  rows[[s]] <- data.frame(
    sigma_u          = sigma_u,
    tau_true         = tau,
    tau_oracle_mean  = mean(tau_oracle_list),
    tau_naive_mean   = mean(tau_naive_list),
    tau_cal_mean     = mean(tau_cal_list),
    tau_placebo_mean = mean(tau_placebo_list),
    tau_oracle_q025  = quantile(tau_oracle_list, 0.025),
    tau_oracle_q975  = quantile(tau_oracle_list, 0.975),
    tau_naive_q025   = quantile(tau_naive_list, 0.025),
    tau_naive_q975   = quantile(tau_naive_list, 0.975),
    tau_cal_q025     = quantile(tau_cal_list, 0.025),
    tau_cal_q975     = quantile(tau_cal_list, 0.975),
    beta_true        = beta,
    beta_oracle_mean = mean(beta_oracle_list),
    beta_naive_mean  = mean(beta_naive_list),
    beta_cal_mean    = mean(beta_cal_list)
  )
...
cat("\n--- Summary (means) ---\n")
print(results %>% select(sigma_u, tau_true, tau_oracle_mean, tau_naive_mean,
                          tau_cal_mean, tau_placebo_mean))

write_csv(results, "outputs/measurement_error_results.csv")

# -----------------------------------------------------------------------------
# Part 3: Plot how measurement error changes estimates
# -----------------------------------------------------------------------------
# Plot tau estimates vs sigma_u
png("figures/measurement_error_tau_vs_sigma.png", width = 800, height = 500)
plot(results$sigma_u, results$tau_oracle_mean, type = "b", pch = 16,
     ylim = range(c(results$tau_oracle_mean, results$tau_naive_mean,
                     results$tau_cal_mean, results$tau_placebo_mean, tau)),
     xlab = "Measurement error SD (sigma_u)",
...
lines(results$sigma_u, results$tau_cal_mean, type = "b", pch = 15, col = "blue")
lines(results$sigma_u, results$tau_placebo_mean, type = "b", pch = 18, col = "purple")
abline(h = tau, lty = 2, col = "gray40")
legend("topleft",
       legend = c("Oracle: y ~ d + x_true", "Naive: y ~ d + x_obs",
                   "Calibration: y ~ d + x_hat", "Outcome placebo: y_pl ~ d + x_obs",
                   "True tau"),
       col = c("black", "red", "blue", "purple", "gray40"),
       pch = c(16, 17, 15, 18, NA), lty = c(1, 1, 1, 1, 2), bty = "n")
dev.off()

# Plot beta estimates vs sigma_u (confounder coefficient attenuation)
png("figures/measurement_error_beta_vs_sigma.png", width = 800, height = 500)
plot(results$sigma_u, results$beta_oracle_mean, type = "b", pch = 16,
     ylim = range(c(results$beta_oracle_mean, results$beta_naive_mean,
                     results$beta_cal_mean, beta)),
     xlab = "Measurement error SD (sigma_u)",
...
lines(results$sigma_u, results$beta_cal_mean, type = "b", pch = 15, col = "blue")
abline(h = beta, lty = 2, col = "gray40")
legend("topright",
       legend = c("Oracle: coef on x_true", "Naive: coef on x_obs",
                   "Calibration: coef on x_hat", "True beta"),
       col = c("black", "red", "blue", "gray40"),
       pch = c(16, 17, 15, NA), lty = c(1, 1, 1, 2), bty = "n")
dev.off()

# -----------------------------------------------------------------------------
# Part 4: Treatment permutation placebo (randomization inference)
# -----------------------------------------------------------------------------
# Observed estimate as test statistic:
#   tau_hat_obs = coef on d in y ~ d + x_obs
# Then build a null distribution by permuting d.
...
write_csv(perm_df, "outputs/permutation_tau_distribution.csv")

# Plot permutation distribution + observed line
png("figures/permutation_placebo_tau_hist.png", width = 800, height = 500)
hist(tau_perm, breaks = 30, col = "lightblue",
...
legend("topright",
       legend = paste0("Observed tau_hat = ", round(tau_hat_obs, 3)),
       lty = 2, col = "red", bty = "n")
dev.off()
